...
use egui_vulkano_backend::EguiVulkanoRenderPass;
use epi::{App, Frame, };
...
struct OffScreenRenderer {
    ...
    frame_buffer: Arc<AttachmentImage>,
    texture_id: TextureId,
    size: [u32; 2],
}

impl OffScreenRenderer {
    fn init(render_pass: &mut EguiVulkanoRenderPass,_device:Arc<Device>) -> Self {
        let (id, image) = render_pass
            .init_vulkano_texture_with_dimensions([1, 1])
            .unwrap();
    ...
    fn resize(&mut self, render_pass: &mut EguiVulkanoRenderPass, dimensions: [u32; 2]) {
        if self.size != dimensions {
            let image = render_pass
                .recreate_vulkano_texture_with_dimensions(self.texture_id, dimensions)
                .unwrap();
...
impl App for OffScreenRenderer {
    fn update(&mut self, ctx: &CtxRef, _frame: &mut Frame<'_>) {
        let window = egui::Window::new("Render triangle");

        window.resizable(true).show(ctx, |ui| {
            ui.vertical(|ui| {
            //    ui.add(egui::Slider::f32(&mut self.rot, -PI..=PI).text("rotation"));
                ui.image(
                    self.texture_id,
...
fn main() {
    ...
    let mut demo_app = OffScreenRenderer::init(&mut egui_render_pass,device.clone());
